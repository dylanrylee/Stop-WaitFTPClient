...
public class StopWaitDriver {
	...
	public static void main(String[] args) {
		...
		int rtxTimeout = Integer.parseInt( params.getOrDefault("-t", "100") ); // duraiton of retransmission time-out interval in milli-seconds
		int connTimeout = Integer.parseInt( params.getOrDefault("-c", "10000") ); // duraiton of connecyion time-out interval in milli-seconds
		Level logLevel = Level.parse( params.getOrDefault("-v", "off").toUpperCase() ); // log levels: all, info, off

		// set log level
		setLogLevel(logLevel);
		
		// send the file
		StopWaitFtp ftp = new StopWaitFtp(connTimeout, rtxTimeout);
